...
1. **Migraciones destructivas aceptadas**: si `device_configs` se reinicia, no es problema; el código debe autocrear defaults y no bloquearse.
2. **Calibración (Opción B)**: se rompe compatibilidad; se adopta un único modelo de calibración compatible con `CalibrationConfig`.
3. **Tiempos**: `startedAt` se mantiene como fecha de creación; se añade `lastActivityAt` para actividad real.
4. **Entorno**: existe `.env.local` configurado; no se añaden nuevas variables salvo necesidad explícita.
...
- [x] **Step 1.1**: Modificar `prisma/schema.prisma` (Match lifecycle + lock)
...
- [x] **Step 1.2**: Crear modelo `Device` en `prisma/schema.prisma`
...
- [x] **Step 1.3**: Hacer `DeviceConfig` multi-dispositivo
    - **Details**: Cambiar `DeviceConfig` de single-row (`id Int @default(1)`) a PK `deviceId String @id`. Se asume migración destructiva permitida: si se pierden datos, la app recrea config por defecto. Actualizar actions para operar por `deviceId` y crear fila si no existe.
    - **Verification**: Migración aplica y `getDeviceConfig(deviceId)` devuelve config.

- [x] **Step 1.4**: Crear migración SQL segura
    - **Details**: Crear `prisma/migrations/*_match_setup_and_device_lock/migration.sql` para añadir columnas a `matches`, crear `devices` y recrear/ajustar `device_configs` a por-dispositivo (destructivo permitido). Mantener el SQL simple y compatible con Postgres.
    - **Verification**: `pnpm prisma migrate dev` aplica sin bloquear la app con tabla vacía.

### Phase 2: Identidad de dispositivo (cliente) y registro (servidor)

- [x] **Step 2.1**: Crear `src/lib/device/device-id.ts`
...
- [x] **Step 2.2**: Crear Server Action `src/app/actions/devices.ts`
...
- [x] **Step 2.3**: Tipos de actions y modelos
...
- [x] **Step 3.1**: Cambiar `createMatch()` a `status: "setup"`
...
- [x] **Step 3.2**: Promover a `ongoing` en el primer throw
    - **Details**: En `registerThrowForPlayer()` y `registerThrow()`, dentro de transacción: si `Match.status="setup"`, al crear el primer `Throw` actualizar `Match.status="ongoing"`. No tocar `startedAt` (fecha de creación). Setear `lastActivityAt=throw.timestamp`.
    - **Verification**: Match pasa de `setup` a `ongoing` al primer dardo.

- [x] **Step 3.3**: Mantener finalización robusta
...
- [x] **Step 4.1**: Crear helper puro `src/lib/device/device-lock.ts`
...
- [x] **Step 4.2**: Extender payload de throws con `deviceId`
...
- [x] **Step 4.3**: Enforzar lock en `registerThrow*`
...
- [x] **Step 4.4**: Acciones explícitas de control
...
- [x] **Step 4.5**: Takeover admin (manual)
...
- [x] **Step 5.1**: Filtrado por estado en `src/app/(admin)/matches/page.tsx`
...
- [x] **Step 5.2**: Acciones por fila (reanudar/abortar/takeover)
...
- [x] **Step 5.3**: Server Action `abortMatch(matchId)`
...
- [x] **Step 5.4**: Descubrimiento multi-dispositivo
...
- [x] **Step 6.1**: Unificar el modelo de calibración
    - **Details**: Adoptar un único shape de calibración basado en `CalibrationConfig` (`centerX/centerY/scale/rotation`). Romper compatibilidad: descartar datos antiguos (`matrix/scaleX/...`). Actualizar Zod schema y acciones para validar/guardar el nuevo shape; si no hay calibración guardada, usar auto-calibración como fallback.
    - **Verification**: La app no se bloquea con calibración vacía o inválida.

- [x] **Step 6.2**: Hacer `getDeviceConfig(deviceId)`
...
- [x] **Step 6.3**: Consumir calibración real en juego
...
- [x] **Step 6.4**: Actualizar `CalibrationModal` para deviceId
    - **Details**: Obtener `deviceId` (lib) y guardar calibración en `updateCalibration(deviceId, data)`. Calcular `centerX/centerY/scale/rotation` en coordenadas del tablero (relative al `DOMRect` del área de diana) para que el juego use el mismo sistema. Ajustar copy: “Calibración de este dispositivo”.
    - **Verification**: Guardar calibración no pisa la de otro dispositivo.

- [x] **Step 6.5**: Alinear `prisma/seed.ts` con el nuevo schema
    - **Details**: Actualizar el seed para que `DeviceConfig.calibration` y `DeviceConfig.preferences` cumplan los schemas actuales (calibración `CalibrationConfig`, preferencias con defaults válidos). Mantener el `deviceId` por defecto.
    - **Verification**: `pnpm prisma db seed` funciona sin errores.

### Phase 7: Observabilidad y protección adicional

- [x] **Step 7.1**: Añadir `lastActivityAt` en cada throw
...
- [x] **Step 7.2**: Respuesta de error estructurada para lock
...
- [x] **Step 8.3**: Documentación de arquitectura
