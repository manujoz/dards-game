...
- [ ] **Step 1.1**: Actualizar `prisma/schema.prisma`
...
- [ ] **Step 1.2**: Añadir dependencias de seguridad en `package.json`
...
- [ ] **Step 1.3**: Añadir secreto de sesión a `.env.local`
...
- [ ] **Step 2.1**: Crear utilidades de sesión en `src/lib/auth/session.ts`
...
- [ ] **Step 2.2**: Crear middleware global `src/middleware.ts`
...
- [ ] **Step 2.3**: Añadir helper de autorización para Server Actions
...
- [ ] **Step 3.1**: Crear schemas de auth en `src/lib/validation/auth.ts`
...
- [ ] **Step 3.2**: Crear Server Actions en `src/app/actions/auth.ts`
...
- [ ] **Step 3.3**: Añadir `logout()` en `src/app/actions/auth.ts`
...
- [ ] **Step 3.4**: Crear componente `src/components/home/AdminLoginForm.tsx`
...
- [ ] **Step 3.5**: Modificar `src/app/page.tsx` para UI condicional
...
- [ ] **Step 4.1**: Proteger `src/app/actions/players.ts`
...
- [ ] **Step 4.2**: Proteger `src/app/actions/matches.ts`
...
- [ ] **Step 4.3**: Proteger `src/app/actions/device-config.ts`
...
- [ ] **Step 4.4**: Revisar lecturas sensibles (opcional)
...
- [ ] **Step 5.1**: UI de creación de admin con contraseña
...
- [ ] **Step 5.2**: Validación y hash de contraseña al crear admin
...
- [ ] **Step 5.3**: Ajustar copy/ayudas en dialogs
...
- [ ] **Step 5.4**: Añadir sección “Cuenta/Seguridad”
...
- [ ] **Step 5.5**: Server Action para cambiar password
...
- [ ] **Step 5.6**: Añadir “Cerrar sesión” en `AdminSidebar.tsx`
...
- [ ] **Step 6.1**: Añadir tests básicos de sesión en `tests/auth/session.test.ts`
    - **Details**: Probar firma/verificación JWT con secreto de test y que `admin:true` es requisito.
    - **Verification**: `pnpm test` pasa.

- [ ] **Step 6.2**: Documentar setup en `README.md` o `docs/`
...
- [ ] **Step 6.3**: Instrucciones para hashear contraseña inicial (manual)
    - **Details**: Añadir un script simple `scripts/hash-password.ts` (o doc con snippet) usando `bcryptjs.hash(password, saltRounds)` que imprima el hash para pegarlo en `players.password`.
